...
class EndlessScreen extends StatefulWidget {
  ...
  EndlessScreen({Key key, this.title}) : super(key: key);

  @override
  _EndlessScreenState createState() {
    return _EndlessScreenState();
  }
}
...
class _EndlessScreenState extends State<EndlessScreen> {
  ...
  double _dtmcount = 0;   //表示用タイマーカウンター
  int _rncount = 0;       //タッチカウンター
  int Mcont = 599;        //タイマーカウント上限値
      ...
      body: Stack(
        children: [
          Container(//背景描画
            decoration: BoxDecoration(
              image: DecorationImage(
                image: AssetImage('assets/images/IMG_6482.JPG'),
                fit: BoxFit.cover,
              ),
            ),
          ),
